...
ap('-method', help='Attack Method', type=str, default="fgsm") 
ap('-file', type=str, default="ckpt_fbit110_Cifar100.t7")
...
if __name__ == '__main__':
    ...
    G_kernel=checkpoint['G_kernels']
    print(checkpoint['epoch'])
    
    total,sp=0.0,0.0
    tw,spw=0.0,0.0
    for kernel in G_kernel:
        for k in range(kernel.size()[1]):
            total+=1
            tw+=torch.numel(kernel[:,k,:,:])
            if kernel[:,k,:,:].sign().abs().sum()==0:
                sp+=1
                spw+=torch.numel(kernel[:,k,:,:])
    print('channel sparsity', sp/total)
    print('sparse weights:',spw)
    tw=0
    for n,p in net.named_parameters():
        tw+=torch.numel(p.data)
...
#-------------------------------------------------------------
    ...
    if attack_type == 'fgsm':
        ...
        for batch_idx, (x1, y1_true) in enumerate(test_loader):
          #if batch_idx < 10:
            if batch_idx - int(int(batch_idx/200.)*200) == 0:
                print(batch_idx)
            x_Test = x1.numpy()
            y_Test = y1_true.numpy()
