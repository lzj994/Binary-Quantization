...
parser.add_argument('--num-ensembles', '--ne', default=3, type=int, metavar='N')
...
if __name__ == '__main__':
    ...
    batchsize_test = len(test_set)/1627 #100 
    ...
    basic_net = en_preactresnet20_cifar(num_ensembles=1, noise_coef=args.noise_coef).cuda()
    #basic_net=densenet.DenseNet(growthRate=12, depth=40, reduction=0.5,bottleneck=False, nClasses=10)
    ...
    #scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[i+10 for i in range(190)],gamma=0.9)

    nepoch = 200
    loss_train=[]
    loss_test=[]
    for epoch in xrange(nepoch):
        print('Epoch ID', epoch, '  eta:', eta)
        ...
        for batch_idx, (x, target) in enumerate(train_loader):
            ...
            for i in range(len(all_W_kernels)):
                ...
                if epoch<120:
                    ...
                    #k_G.data = (eta*quantize_fbit(V)+V)/(1+eta)
                    #k_G.data = (eta*quantize_tnn(V)+V)/(1+eta)

                else:
                    k_G.data = quantize_bw(V)
                    #k_G.data =quantize_fbit(V)
            ...
            ########### Method by Hongyang Zhang etc. ##############
            #loss=trades.trades_loss(model=basic_net,x_natural=x,y=target,optimizer=optimizer,beta=4.0)
            
            ########Trade-off Loss function#######
            loss_natural=criterion(basic_net(x),target)
            score, pert_x = net(x, target)
            loss = criterion(score, target)
            loss=8*loss+loss_natural
            ...
            #predicted=torc.max(score.data,1)
            total += target.size(0)
            correct += predicted.eq(target.data).cpu().sum()
            progress_bar(batch_idx, len(train_loader), 'Loss: %.3f | Acc: %.3f%% (%d/%d)'
            % (train_loss/(batch_idx+1), 100.*correct/total, correct, total))
        loss_train.append(train_loss.cpu().data.numpy()/batchsize_train)
        if epoch<120:
            eta=eta*eta_rate
        ...
        loss_test.append(test_loss.cpu().data.numpy()/batchsize_test)
        #----------------------------------------------------------------------
        # Save the checkpoint
        #----------------------------------------------------------------------
        ...
        if correct > best_count:
            ...
            torch.save(state, './ckpt_bw56_SVHN.t7')
            best_acc = acc
            best_count = correct
